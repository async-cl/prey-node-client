...
/*
  ...
  this.register_device = function(callback){
    logger.info("Attaching device to your account with api key "+this.config.api_key);
    require('./register').new_device({api_key: this.config.api_key}, function(err, data){
      if(err || !data.device_key){
        return callback(_error(err || new Error("Couldn't register this device. Please try again in a sec.")));
      } else {
        _tr('ok here')
