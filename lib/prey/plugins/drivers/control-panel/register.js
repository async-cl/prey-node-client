...
var get_device_data = function(callback){
  ...
  Hardware.get_firmware_info(function(err, hw_info) {
    ...
    System.get_os_name(function(err, os_name){
      ...
      // TODO: registration needs capital W for windows ... this needs rationalized
      // also, Ubuntu variations not recognised
      data.os = os_name.replace('LinuxMint','Ubuntu').replace('windows','Windows');

      System.get_os_version(function(err, os_version) {
        _tr('got version',os_version)
        if (err) return callback(_error(err));
